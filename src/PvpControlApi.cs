...
namespace ErenshorPvP
{
    ...
    public static class PvpControlApi
    {
        ...
        public static bool RuntimeReady
        {
            get
            {
                ErenshorPvPPlugin plugin = ErenshorPvPPluginHolder.Instance;
        ...
        public static PvpControlState GetBasicState()
        {
            ...
            state.GameplayReady = RuntimeReady && SuiteUiPolicy.IsGameplayReady();
        ...
        {
            ErenshorPvPPlugin plugin = ErenshorPvPPluginHolder.Instance;
            if (plugin == null) return "PvP unavailable";
            ...
            return PvpController.HubStatus();
        }
        ...
        public static bool AcceptPending() { if (!RuntimeReady || !PvpController.HasPending) return false; PvpController.Accept(); return true; }
        public static bool RefusePending() { if (!RuntimeReady || !PvpController.HasPending) return false; PvpController.Refuse(); return true; }
        public static string FleeEncounter() { return RuntimeReady && PvpTemporaryCloneFactory.HasActiveTeam ? PvpTemporaryCloneFactory.Flee() : "No active PvP encounter."; }
        public static string SetAmbushHere(bool enabled) { return RuntimeReady ? PvpController.SetAmbushHere(enabled) : "PvP compatibility unavailable."; }
        ...
        public static bool SetEnabled(bool enabled) { if (!RuntimeReady || !SuiteUiPolicy.IsGameplayReady()) return false; PvpController.SetEnabled(enabled); return true; }
        public static bool SetArrangedEnabled(bool enabled) { if (!RuntimeReady) return false; PvpController.SetArrangedEnabled(enabled); return true; }
        public static bool SetAmbushEnabled(bool enabled) { if (!RuntimeReady) return false; PvpController.SetAmbushEnabled(enabled); return true; }
    }
}
